...
class CloudCompare:
    ...
    def CCcommand(self, commands, outName):
        """
        ...
        if os.path.isdir(self.dir) == True:
            ...
            tmpFilePath = max(glob.iglob(os.path.splitext(filePath)[0]+'*'), key=os.path.getctime)

            #outputFilePath
            outputFilePath = os.path.join(os.path.dirname(filePath), outName + os.pathsplitext(tmpFilePath)[1])
    ...
    def CCcommandE57(self, filePath, commands, outName):
        ...
        if os.path.isdir(self.dir) == True:
            ...
            if os.path.isfile(tmpFilePath)
                ...
                return 'CloudCompare CommandLine Failed'
        else:
            return 'CloudCompare missing. Install CloudCompare in ' + self.dir
    ...
    def toXYZ(self, filePath):
        ...
        if os.path.isdir(self.dir) == True:
            #run CloudCompare cmd line
            Args = self.dir + '\CloudCompare -O ' + filePath + ' -NO_TIMESTAMP -C_EXPORT_FMT ASC -EXT xyz -MERGE_CLOUDS -SAVE_CLOUDS' 
            P = subprocess.Popen(Args , cwd=self.dir)
            P.wait()
            
            #outputFilePath
            tmpFilePath = max(glob.iglob(os.path.splitext(filePath)[0]+'*.xyz'), key=os.path.getctime)
            outputFilePath = os.path.splitext(filePath)[0] + '.xyz'

            mergePath = os.path.splitext(filePath)[0] + '_MERGED.xyz'
            if tmpFilePath == mergePath:
                #Rename (and move) tmpFilePath
                shutil.move(tmpFilePath, outputFilePath)
